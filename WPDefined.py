...
def load_training_file(trainFile, maxlength, train_file_style):
    def load_train_trainOfSenti(file):   
        ...
        for line in senti_file:
            ...
            data.append(sent)
        senti_file.close()
        print '\nVocab:'+str(word_count-1)+', '+str(sent_count)+' sentences read.'
        ...
        return numpy.array(data),numpy.array(Lengths), numpy.array(leftPad),numpy.array(rightPad), word_count, id2word, numpy.array(unigram)
    ...
    def load_train_wikipedia(file):   
        ...
        for line in senti_file:
            if len(line.strip())>0: #not empty lines
                ...
                data.append(sent)
        senti_file.close()
        print '\nVocab:'+str(word_count-1)+', '+str(j)+' sentences read.'
        #compute word occurring probability
        all_tokens=sum(Lengths)
        for key in id2count:
            id2count[key]=id2count[key]*1.0/all_tokens  #discard the first zero word
    ...
    if train_file_style != 0:
        indices_train, trainLengths, trainLeftPad, trainRightPad, word_count, id2word, unigram=load_train_trainOfSenti(trainFile)
    elif train_file_style == 0:
        indices_train, trainLengths, trainLeftPad, trainRightPad, word_count, id2word, unigram=load_train_wikipedia(trainFile)
    ...
    rval = (indices_train,train_set_Lengths, train_left_pad, train_right_pad)
    return rval, unigram, trainLengths, word_count, id2word
def load_data_for_training(trainFile, devFile, maxlength):
    ...
    def load_train(file):   
        ...
        for line in senti_file:
            if len(line.strip())>0: #not empty lines
                words=line.strip().split() #convert to lowercase
                if len(words)<1:
                    continue
                sent=[]
                length=len(words)
    ...
    def load_test(file, word2id, word_count):   
        senti_file=open(file, 'r')
        data=[]
        #total_lines=74915588 
        Lengths=[]
        leftPad=[]
        rightPad=[]
        word_count+=1 #word_count+1 in the beginning
        for line in senti_file:
            if len(line.strip())>0: #not empty lines
                words=line.strip().split() #convert to lowercase
                sent=[]
        ...
        word_count=word_count-1    
        #store_word2id_into_file(word2id)
        return numpy.array(data),numpy.array(Lengths), numpy.array(leftPad),numpy.array(rightPad), word_count, word2id

    indices_train, trainLengths, trainLeftPad, trainRightPad, word_count, word2id, unigram=load_train(trainFile)
    print 'train file loaded over, sentence totally:', str(len(trainLengths)), 'words:', len(word2id)
    indices_test, testLengths, testLeftPad, testRightPad, word_count, word2id=load_test(devFile, word2id, word_count)
    print 'test file loaded over, sentence totally:', str(len(testLengths)), 'words:', len(word2id)
    ...
    train_set_Lengths=shared_dataset(trainLengths)                             
    valid_set_Lengths = shared_dataset(testLengths)
    #uni_gram=shared_dataset(unigram)
    
    train_left_pad=shared_dataset(trainLeftPad)
    train_right_pad=shared_dataset(trainRightPad)
    dev_left_pad=shared_dataset(testLeftPad)
    dev_right_pad=shared_dataset(testRightPad)
    

    rval = [(indices_train,train_set_Lengths, train_left_pad, train_right_pad), (indices_test, valid_set_Lengths, dev_left_pad, dev_right_pad)]
    return rval, unigram, trainLengths, testLengths,word_count, id2word
...
class Conv_Fold_DynamicK_PoolLayer(object):
    #now, change it to having no folding operation, so that its one-dimensional convolution
...
class Conv_Thang(object):
    """this convolution function comes from sent_classify_onelayer """
    
        
    def __init__(self, rng, input, filter_shape, image_shape, poolsize=(2, 2), k=[], unifiedWidth=30, left=[], right=[], firstLayer=True, activation=T.tanh):
        ...
        b_values = numpy.zeros((filter_shape[0],), dtype=theano.config.floatX)
        self.b = theano.shared(value=b_values, borrow=True)
        #wide conv in column direction, and narrow conv in row direction
        conv_out_raw=  conv.conv2d(input=input, filters=self.W, filter_shape=filter_shape, image_shape=image_shape, border_mode='full')
        embedding_size=filter_shape[2]
        conv_out_tensor=conv_out_raw[:,:,(embedding_size-1):embedding_size,:] # produce (batchsize, kern, 1, columns)
        fold_out=debug_print(conv_out_tensor.reshape((image_shape[0], 1, filter_shape[0], poolsize[1])),'fold_out') # produce (batchsize, 1, kern, columns)
        # convolve input feature maps with filters
        #conv_out = conv.conv2d(input=input, filters=self.W,
        #        filter_shape=filter_shape, image_shape=image_shape, border_mode='full')
        #folding
        ...
        for i in range(image_shape[0]): # image_shape[0] is actually batch_size
            ...
            non_zeros_raw=debug_print(neighborsForPooling[:,left[i]:(neighborsForPooling.shape[1]-right[i])],'non_zeros_raw') # only consider non-zero elements
            non_zeros=debug_print(repeat_whole_matrix(non_zeros_raw, 10, False),'non_zeros')
        ...
        #@wenpeng:  following tanh operation will voilate our expectation that zero-padding, for its output will have no zero any more
        #self.output = T.tanh(pooled_out + self.b.dimshuffle('x', 0, 'x', 'x'))
        biased_pooled_out=pooled_out + self.b.dimshuffle('x', 'x', 0, 'x')
        ...
        if firstLayer:
            ...
            overall_matrix_new=T.concatenate(zero_recover_matrices, axis=0)  
            pooled_out_with_zeros = T.reshape(overall_matrix_new, new_shape, ndim=4) 
            self.output=debug_print(activation(pooled_out_with_zeros),'output')
        else:
            self.output=activation(biased_pooled_out)

        # store parameters of this layer
        self.params = [self.W, self.b]

class Full_Convolayer(object):
    #this function is used to replace the function in implementation of Oxford method: concatenating a window of words to produce another ngram embedding
    def __init__(self, rng, input, filter_shape, image_shape, poolsize=(2, 2), k=[], unifiedWidth=30, left=[], right=[], newd=100, firstLayer=True):
        ...
        new_weight_shape=(newd, filter_shape[0], filter_shape[1], filter_shape[2], filter_shape[3])
        self.W = theano.shared(numpy.asarray(
            rng.uniform(low=-W_bound, high=W_bound, size=new_weight_shape),
            dtype=theano.config.floatX), borrow=True)

        # the bias is a 1D tensor -- one bias per output dimension
        b_values = numpy.zeros((newd,1), dtype=theano.config.floatX)
        self.b = theano.shared(value=b_values, borrow=True)
        bb=T.repeat(self.b, unifiedWidth, axis=1)
        conv_out_list=[]
        for d in range(newd):
            conv_out_d = debug_print(conv.conv2d(input=input, filters=self.W[d], filter_shape=filter_shape, image_shape=image_shape, border_mode='valid'), 'conv_out_d') # only valid filter produces one value
            conv_out_list.append(conv_out_d)
        fold_out=debug_print(T.concatenate(conv_out_list, axis=2), 'fold_out') # produce 4d tensor (batch_size, feature_maps, newd, col)           
        ...
        for i in range(image_shape[0]): # image_shape[0] is actually batch_size
            ...
            non_zeros=debug_print(neighborsForPooling[:,left[i]:(neighborsForPooling.shape[1]-right[i])], 'non_zeros') # only consider non-zero elements
            #wenpeng2=theano.printing.Print('non-zeros')(non_zeros)
