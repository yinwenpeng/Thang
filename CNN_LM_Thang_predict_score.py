...
class CNN_LM(object):
    def __init__(self, learning_rate=0.2, n_epochs=2000, nkerns=[6, 14], batch_size=10, useAllSamples=False, ktop=4, filter_size=[7,5],
    ...
    #                    L2_weight=0.000005, dropout_p=0.5, useEmb=0, task=5, corpus=1):
        ...
        #score of Thang
        score=T.nnet.sigmoid(delta_s_theta_data)
        ...
        validate_model = theano.function([index,iteration], [self.cost,score],
                givens={
        ...
        # TRAIN MODEL #
        ###############
        print '... predicting ', len(dev_batch_start), ' sentences.'
        if not self.from_scratch:
            self.load_model_from_file()#reset the model parameters
        
        score_file=open('/mounts/data/proj/wenpeng/Thang/score_file.txt','w')
        validation_losses=[]           
        test_pp=0.0
        for batch_start in dev_batch_start:
            total_iteration=max(self.vali_lengths[batch_start: batch_start + self.batch_size]) #sentence length
            sent_score=0.0
            sent_pp=1.0
            for iteration in range(total_iteration): #for each word
                vali_loss_i, score_i=validate_model(batch_start, iteration)
                sent_score+=score_i
                sent_pp*=1.0/score_i
                validation_losses.append(vali_loss_i)   
            sent_pp=math.pow(sent_pp, 1.0/total_iteration)
            test_pp+=sent_pp
            score_file.write(str(sent_score)+'\t'+str(sent_pp)+'\n')    
        this_validation_loss = numpy.mean(validation_losses)
        test_pp=test_pp/len(dev_batch_start)
        print 'test loss: ', this_validation_loss, 'PP:', test_pp, ' scores written over.'
        score_file.close()
...
if __name__ == '__main__':
    ...
    network=CNN_LM(learning_rate=0.005, nkerns=[1, 1], batch_size=1, ktop=1, filter_size=[7,5],
