...
class CNN_LM(object):
    ...
    def load_pretrained_embeddings(self):
        ...
        read_file=open('/mounts/data/proj/wenpeng/Sent2Vec/context_target_embeddings_lr0.0005_nk3&4_bs5_fs7&5_maxSL60_win3_noi10_wait5_wdEm4_stEm8_tsTrue_newd4&6.txt')
        for line in read_file:
            tokens=line.strip().split('\t')
            word2embeddings_R[tokens[0]]=tokens[1].split()
        ...
        print 'Embedding initialize over, '+str(unknown_words)+' unknown words'
    ...
    def load_model_from_file(self):
        save_file = open('/mounts/data/proj/wenpeng/Thang/model_params_lr0.0005_nk3&4_bs5_fs7&5_maxSL60_win3_noi10_wait5_wdEm4_stEm8_tsTrue_newd4&6')
    ...
    #                    L2_weight=0.000005, dropout_p=0.5, useEmb=0, task=5, corpus=1):
        ...
        patience = 5000000000  # look as this many examples regardless
        ...
        while (epoch < self.n_epochs) and (not done_looping):
            ...
            for batch_start in train_batch_start: 
                ...
                iteration=numpy.random.randint(total_iteration)
                average_cost_per_batch=train_model(batch_start, iteration)
                '''
                # we only care the last cost within those iterations
                cost_of_each_iteration=0.0
                costs_in_batch=[]
                for iteration in range(total_iteration):
                    #print 'iteration: '+str(iteration)+'/'+str(total_iteration)+' in iter '+str(iter)
                    cost_of_each_iteration = train_model(batch_start, iteration)
                    costs_in_batch.append(cost_of_each_iteration)
                ...
                if iter % validation_frequency == 0:
                    ...
                    if this_validation_loss < (minimal_of_list(vali_loss_list)-self.stop):
                        del vali_loss_list[:]
                        vali_loss_list.append(this_validation_loss)
                        lowest_vali_loss=this_validation_loss
                        #remember the best params
                        self.best_params=self.params_to_store
                        self.best_embeddings_R=self.embeddings_R
                        self.best_embeddings_Q=self.embeddings_Q
                        #self.best_sentence_embs=sentences_embs
                        ...
                        if len(vali_loss_list)==self.wait_iter:
                            ...
                            #self.store_sentence_embeddings()
                            self.store_context_target_embeddings()
                            print 'Training over, best model got at valid_cost:'+str(lowest_vali_loss)
                            exit(0) 
        ...
        self.store_model_to_file()
        self.store_context_target_embeddings()
        print 'Training over, best model got at train_cost:'+str(lowest_vali_loss)
...
if __name__ == '__main__':
    ...
    network=CNN_LM(learning_rate=0.005, nkerns=[1, 1], batch_size=50, ktop=1, filter_size=[7,5],
                    L2_weight=0.00005, maxSentLength=60, sentEm_length=200, window=3, 
                    k=10, only_left_context=True, wait_iter=10, embedding_size=200, newd=[200, 200], train_file_style=0, from_scratch=True, stop=1e-3)
